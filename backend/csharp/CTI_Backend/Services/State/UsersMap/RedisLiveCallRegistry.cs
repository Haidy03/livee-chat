// ============================================================================
// COPIED VERBATIM from VoiceFlow.Reports.Infrastructure.Telemetry.RedisLiveCallRegistry.
// Only adjustments: namespace + RedisOptions import path. Logic, key layout,
// and behavior are byte-for-byte identical to the original.
// Source: backend/csharp/ReportsService/.../Telemetry/RedisLiveCallRegistry.cs
// ============================================================================
...
public sealed class RedisLiveCallRegistry : ILiveCallRegistry
{
    ...
    public async Task RecordStateAsync(LiveCall call, CancellationToken ct)
    {
    ...
    public async Task<LiveCallsSnapshot> GetSnapshotAsync(string tenantId, CancellationToken ct)
    {
        ...
        var calls = new List<LiveCall>(activeIds.Length);
        if (activeIds.Length > 0)
        {
            ...
            for (int i = 0; i < values.Length; i++)
            {
                ...
                try
                {
                    var call = JsonSerializer.Deserialize<LiveCall>(values[i]!, Json);
