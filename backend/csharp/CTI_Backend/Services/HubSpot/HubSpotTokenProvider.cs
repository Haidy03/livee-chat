...
public sealed class HubSpotTokenProvider : IHubSpotTokenProvider
{
    ...
    public async Task<string> GetValidAccessTokenAsync(string tenantId, CancellationToken ct)
    {
        ...
        try
        {
            return await RefreshAsync(i);
        }
        catch(Exception ex)
        {
           throw new HubSpotOAuthException("HUBSPOT_REFRESH_FAILED", "Failed to refresh access token.");
        }
    ...
    private async Task<string> RefreshAsync(HubSpotIntegration i, CancellationToken? ct = default)
    {
        ...
        using var resp = await client.PostAsync(_opt.TokenUrl, form);
        if ((int)resp.StatusCode is 400 or 401 or 403)
        {
            ...
            await _repo.UpsertIntegrationAsync(i);
        ...
        var tokens = JsonSerializer.Deserialize<HubSpotTokenResponse>(await resp.Content.ReadAsStringAsync())
